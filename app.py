...
def estimate_chemical_emission(activity_id, weight_kg, api_key=CLIMATIQ_API_KEY):
    ...
    try:
        ...
        if co2e is None:
            print(f"No CO2e returned for {activity_id}. Response: {result}")
        return co2e
    except requests.exceptions.RequestException as e:
        print(f"API request failed for {activity_id} ({weight_kg} kg): {e}")
        return None
...
if uploaded_file:
    ...
    # Activity mapping (check supported HFCs)
    activity_mapping = {
        "HFC-227ea": "fugitive-hfc-227ea",
        "HFC-23": "fugitive-hfc-23",
        "HFC-236fa": "fugitive-hfc-236fa",  # may be unsupported
        "HFC-125": "fugitive-hfc-125",
        "HFC-143a": "fugitive-hfc-143a",
        "HFC-134a": "fugitive-hfc-134a",    # may be unsupported
    ...
    # Estimate CO2e for each activity
    co2e_results = {}
